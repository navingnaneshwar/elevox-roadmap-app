...
// Reads an active clarification_session, renders questions one-by-one,
// saves answers, then queues build_framework when all high-priority answered.
...
export default function ClarificationPage() {
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: '#070B14', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter', sans-serif", padding: '40px' }}>
      <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(74,158,255,0.1)', border: '1px solid rgba(74,158,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', marginBottom: '24px' }}>
        ✓
      </div>
      <div style={{ fontSize: '22px', fontWeight: '700', color: '#F1F5F9', fontFamily: "'Outfit', sans-serif", marginBottom: '12px', textAlign: 'center' }}>
        Building your framework
      </div>
      <div style={{ fontSize: '13px', color: '#64748B', textAlign: 'center', maxWidth: '420px', lineHeight: '1.7', marginBottom: '32px' }}>
        Chanakya is now synthesising your answers with today's industry signals to craft your personalised 90-day brand framework. This takes 1–2 minutes.
      </div>
      <button
        onClick={() => navigate('/dashboard')}
        style={{ padding: '11px 28px', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '8px', color: '#a5b4fc', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}
      >
        Back to Dashboard →
      </button>
    </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070B14', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '60px 40px 80px', position: 'relative', zIndex: 1 }}>
        ...
        {currentIdx === 0 && intro && (
          <div style={{ marginBottom: '36px', padding: '20px 24px', background: 'rgba(99,102,241,0.05)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '12px' }}>
            <div style={{ fontSize: '10px', letterSpacing: '2.5px', color: '#6366f1', marginBottom: '8px', fontWeight: '600' }}>FROM CHANAKYA</div>
            <div style={{ fontSize: '14px', color: '#94a3b8', lineHeight: '1.7' }}>{intro}</div>
          </div>
        )}
